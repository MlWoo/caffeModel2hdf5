...
classifier3.name = 'loss3/classifier'
...
conv1_1x1_s1_copy.name = 'loss2/conv'
...
--[[
local size = 224
local input = torch.randn(1, 3, size, size):fill(0)
...
if test then
  ...
  local count = 0
  local batchSize = 32
  local input = torch.Tensor(batchSize, 3, size, size)
  ...
  local mean = torch.Tensor(input:size())
  mean[{{}, {1}, {}, {}}] = -104
  mean[{{}, {2}, {}, {}}] = -117
  mean[{{}, {3}, {}, {}}] = -123

  local index = 1
  --[[local dirs = dir.getdirectories('/data/imagenet/val/')
  table.sort(dirs)
  print(dirs)
  for file in paths.iterfiles("/data/imagenet/val/") do
      print(file)
  end
  local index = 1
  print('---------')]]--
